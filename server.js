...
const app = express();
const upload = multer({ dest: 'uploads/' });  // Temp uploads dir
...
app.post('/api/analyze', upload.single('file'), async (req, res) => {
  ...
  if (urlMatch) {
    try {
      ...
      scraped = $('body').text().slice(0, 2000);  // Basic manifest extract
...
async function runFlipBotAnalysis(items, fullText) {
  // Live scraping (basic eBay/Google example—expand for full)
  let soldCount = 200;
  let avgPrice = 180;
  try {
    ...
    soldCount = parseInt(snippet.match(/\d+/)?.[0]) || 200;  // Parse snippet
    avgPrice = parseInt(snippet.match(/\$(\d+)/)?.[1]) || 180;
  } catch (e) {
    // Fallback
  }

  const trendsScore = 75;  // Mock Trends (add API key for real)
  const shipCA = 280;  // Est. from location
  const shipWA = 350;

  // ROI Calc (3x goal, 1 month)
  const markup = 3;
  ...
  return {
    'Good Buy?': 'Maybe - Untested risk but high demand for 3x ROI in budget.',
